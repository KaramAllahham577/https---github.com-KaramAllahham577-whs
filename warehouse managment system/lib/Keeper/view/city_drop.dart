...
class CityDropdown extends StatelessWidget {
  final CityController addressController = Get.put(CityController());
  final bool co;

  CityDropdown({super.key, required this.co});
        ...
        return DropdownButtonFormField<Address>(
          ...
          items: [
             DropdownMenuItem<Address>(
              value: null,
              child: co?const Text("Select Source City"):const Text("Select Destination City"),
            ),
            ...addresses.map<DropdownMenuItem<Address>>((Address address) {
              return DropdownMenuItem<Address>(
                value: address,
                child: Text(address.region.city.city),
              );
            }).toList(),
          ],
        );
